...
output_dir    = 'outputs/caez5g/ORU0/avg6_l11_l12/'
...
# L1 distance computation over window
def _manhattan_slot(hi):
...
def compute_l11_window_distance_matrix(H_feat, avg_axis):
    """Compute L1,1 over corresponding temporal samples."""
    N      = H_feat.shape[0]
    ...
    for slc in tqdm(outer_slices, desc=f"  L_1,1 ({avg_axis})", leave=False):
        ...
        for k in range(Kslots):
            ...
            D_sum += _manhattan_slot(hi)                 # accumulate |Dd|

    # arithmetic mean over all (n_outer * K) terms  (L_{1,1})
    D = D_sum / total_terms
    np.fill_diagonal(D, 0.0)
    return D


def compute_l12_window_distance_matrix(H_feat, avg_axis):
    """Compute L1,2 over corresponding temporal samples."""
    N      = H_feat.shape[0]
    n_dmrs = H_feat.shape[3]   # 3
    n_win  = H_feat.shape[4]
    Kslots = n_dmrs * n_win    # 18
    ...
    for slc in tqdm(outer_slices, desc=f"  L_1,2 ({avg_axis})", leave=False):
        ...
        for k in range(Kslots):
            hi = slc_K[:, k, :].astype(np.complex128)   # (N, vec_dim)
            Dd = _manhattan_slot(hi)                     # (N, N) scalar distances
            D_sq2 += Dd ** 2                             # accumulate D^2 — no sqrt yet

    # single sqrt over mean of all squared distances  (L_{1,2})
    D = np.sqrt(D_sq2 / total_terms)
    np.fill_diagonal(D, 0.0)
    return D
...
# Only Manhattan distance — the sole L_{p=1} compatible distance.
configs = [
    (
        'Antenna-Frequency H (CFR, ORU0, avg6-L11-L12)',
        H_win,
        'metrics_H_antenna_freq_CFR_273sc_4ant_ORU0_avg6_l11_l12.npz',
        [('col', 'antenna_per_freq')]
    ),
    (
        'Antenna-Delay C (CIR 68taps, ORU0, avg6-L11-L12)',
        C_cir_win,
        'metrics_C_antenna_delay_CIR_68taps_4ant_ORU0_avg6_l11_l12.npz',
        [('row', 'delay_per_antenna'), ('col', 'antenna_per_delay')]
    ),
    (
        'Beam-Delay C_beam (CIR 68taps, ORU0, avg6-L11-L12)',
        C_beam_cir_win,
        'metrics_Cbeam_beam_delay_CIR_68taps_4beam_ORU0_avg6_l11_l12.npz',
        [('row', 'delay_per_beam')]
    ),
]
...
for cfg_label, H_feat, out_fname, feature_types in configs:
    ...
    for feat_type, feat_label in feature_types:
        ...
        D_l11   = compute_l11_window_distance_matrix(H_feat, avg_axis=feat_type)
        ...
        D_l12   = compute_l12_window_distance_matrix(H_feat, avg_axis=feat_type)
