...
output_dir    = 'outputs/caez5g/ORU0/avg6_sqasd/'
...
def make_feature(win_data):
    ...
    return out   # (N, 4, F, N_DMRS=3, N_WINDOW=6)

H_win          = make_feature(CFR_win)                                          # (N, 4, 273, 3, 6)
C_cir_win      = make_feature(CIR_win)                                          # (N, 4,  68, 3, 6)
...
# RASD distance computation over window
def compute_sqasd_window_distance_matrix(H_feat, dist_type, avg_axis):
    ...
    total_terms = n_outer * Kslots          # denominator inside the sqrt
    D_sq2       = np.zeros((N, N), dtype=np.float64)   # accumulates sum of D^2
...
configs = [
    (
        'Antenna-Frequency H (CFR, ORU0, avg6-SqASD)',
        H_win,
        'metrics_H_antenna_freq_CFR_273sc_4ant_ORU0_avg6_sqasd.npz',
        [('col', 'antenna_per_freq')]
    ),
    (
        'Antenna-Delay C (CIR 68taps, ORU0, avg6-SqASD)',
        C_cir_win,
        'metrics_C_antenna_delay_CIR_68taps_4ant_ORU0_avg6_sqasd.npz',
        [('row', 'delay_per_antenna'), ('col', 'antenna_per_delay')]
    ),
    (
        'Beam-Delay C_beam (CIR 68taps, ORU0, avg6-SqASD)',
        C_beam_cir_win,
        'metrics_Cbeam_beam_delay_CIR_68taps_4beam_ORU0_avg6_sqasd.npz',
        [('row', 'delay_per_beam')]
    ),
]
...
for cfg_label, H_feat, out_fname, feature_types in configs:
    ...
    save_dict = {
        ...
        'K_slots':      np.int32(K),
        'aggregation':  np.bytes_('SqASD_L22'),
    }
