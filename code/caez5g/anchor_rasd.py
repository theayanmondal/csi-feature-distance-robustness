...
output_dir    = 'outputs/caez5g/ORU0/anchor_only_sqasd/'
...
N_SC_FULL = 3276
N_SC_CFR  = 273        # every 12th subcarrier
...
for anchor_i in tqdm(anchor_order_by_shard, desc="Loading anchors from shards"):
    ...
    CIR[anchor_i] = ch_cir[:, TAP_START:TAP_END, :].astype(np.complex64)
...
def compute_sqasd_distance_matrix(H_win, dist_type, avg_axis):
    ...
    D_sq2       = np.zeros((N_locs, N_locs), dtype=np.float64)  # accumulates sum of D^2
    ...
    for slc in tqdm(outer_slices, desc=f"  {dist_type} ({avg_axis})", leave=False):
        for d in range(n_dmrs):
            ...
            # scalar distance D_d(i,j) for this slice/DMRS
            ...
            # accumulate D^2 — no sqrt yet
            D_sq2 += Dd ** 2

    # single sqrt over the mean of all squared distances
...
configs = [
    (
        'Antenna-Frequency H (CFR, ORU0, anchor-only, SqASD)',
        H_anchor,
        'metrics_H_antenna_freq_CFR_273sc_4ant_ORU0_anchoronly_sqasd.npz',
        [('col', 'antenna_per_freq')]
    ),
    (
        'Antenna-Delay C (CIR 68taps, ORU0, anchor-only, SqASD)',
        C_cir_anchor,
        'metrics_C_antenna_delay_CIR_68taps_4ant_ORU0_anchoronly_sqasd.npz',
        [('row', 'delay_per_antenna'), ('col', 'antenna_per_delay')]
    ),
    (
        'Beam-Delay C_beam (CIR 68taps, ORU0, anchor-only, SqASD)',
        C_beam_cir_anchor,
        'metrics_Cbeam_beam_delay_CIR_68taps_4beam_ORU0_anchoronly_sqasd.npz',
        [('row', 'delay_per_beam')]
    ),
]
...
for cfg_label, H_win, out_fname, feature_types in configs:
    ...
    for feat_type, feat_label in feature_types:
        ...
        for dt, dl in zip(dist_types, dist_labels):
            D_chan  = compute_sqasd_distance_matrix(H_win, dt, avg_axis=feat_type)
