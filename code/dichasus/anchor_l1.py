...
print("Loading and calibrating all files...")
file_data = []
for file_idx, (tf_file, off_file) in enumerate(zip(tfrecord_files, offset_files)):
    ...
    for sample in tqdm(ds, desc=fname):
        ...
        csi_c = np.fft.fftshift(
            csi_c,
            axes=-1,
        )
        csi_rev2 = normalize_rev2_sto(
            csi_c
        )
        csi_cal = apply_calibration(
            csi_rev2,
            offsets,
        )
        file_csi.append(
            csi_cal[ARRAY_A_CHANNELS, :]
        )
...
print("\nBuilding eligible pool and sampling anchors...")
...
# Extract anchor CFRs
print("\nExtracting anchor CFRs...")
CFR = np.array([
    file_data[anchor_file_idx[i]]["csi"][anchor_local[i]]
    for i in range(TARGET_ANCHORS)
], dtype=np.complex64)   # (4000, 8, 1024)
print(f"CFR shape: {CFR.shape}  dtype: {CFR.dtype}")

# Compute centred CIR and fixed 18-tap beam-delay
print(
    f"\nComputing centred CIR and extracting "
...
print(
    f"C_cir_anchor shape: {C_cir_anchor.shape}  "
    f"dtype: {C_cir_anchor.dtype}"
)

print("Computing 2D beam-delay representation...")
...
print(
    ...
    f"{C_beam_cir_anchor.shape}  "
    f"dtype: {C_beam_cir_anchor.dtype}"
)
...
# L_{1,1} and L_{1,2} distance functions
def _manhattan(hi):
    diff = hi[:, np.newaxis, :] - hi[np.newaxis, :, :]   # (N, N, vec_dim)
    return np.sum(np.abs(diff), axis=-1)                  # (N, N) real

def compute_l11_distance_matrix(H, avg_axis):
    N = H.shape[0]
    if avg_axis == 'row':
        n_outer      = H.shape[1]
    ...
    for slc in tqdm(outer_slices, desc=f"  L_1,1 ({avg_axis})", leave=False):
        D += _manhattan(slc.astype(np.complex128))
    D /= n_outer
    np.fill_diagonal(D, 0.0)
    return D

def compute_l12_distance_matrix(H, avg_axis):
    N = H.shape[0]
    if avg_axis == 'row':
        n_outer      = H.shape[1]
        outer_slices = [H[:, m, :] for m in range(n_outer)]
        ...
        n_outer      = H.shape[2]
        outer_slices = [H[:, :, k] for k in range(n_outer)]
    D_sq2 = np.zeros((N, N), dtype=np.float64)
    for slc in tqdm(outer_slices, desc=f"  L_1,2 ({avg_axis})", leave=False):
        D_sq2 += _manhattan(slc.astype(np.complex128)) ** 2
...
# Configurations
# antenna_per_freq  : CFR,           avg_axis='col' (outer=freq,   inner=antenna)
# antenna_per_delay : CIR 18 taps,  avg_axis='col' (outer=delay,  inner=antenna)
# delay_per_antenna : CIR 18 taps,  avg_axis='row' (outer=antenna,inner=delay)
# delay_per_beam    : beam-CIR,      avg_axis='row' (outer=beam,   inner=delay)
configs = [
    ('Antenna-per-Frequency (CFR, Array A, anchor-only)',  CFR,               'col', 'antenna_per_freq'),
    ('Antenna-per-Delay (CIR fixed 18 taps, Array A, anchor-only)',  C_cir_anchor,      'col', 'antenna_per_delay'),
     ('Delay-per-Antenna (CIR fixed 18 taps, Array A, anchor-only)', C_cir_anchor, 'row', 'delay_per_antenna'),
    ('Delay-per-Beam (2D beam-CIR, fixed 18 taps, Array A, anchor-only)', C_beam_cir_anchor, 'row', 'delay_per_beam'),
...
for cfg_label, H, avg_axis, feat_label in configs:
    ...
    D_l11 = compute_l11_distance_matrix(H, avg_axis=avg_axis)
    feat_results['l11'] = compute_all_metrics(D_phys, D_l11, J)

    print(f"    Computing L_{{1,2}} ...")
    D_l12 = compute_l12_distance_matrix(H, avg_axis=avg_axis)
    feat_results['l12'] = compute_all_metrics(D_phys, D_l12, J)

    all_results[feat_label] = feat_results

# Final summary
print(f"\n{'='*120}")
print("  FINAL SUMMARY — DICHASUS rev2, Array A, anchor-only, fixed taps [509:527], L_{{1,1}} and L_{{1,2}}")
print(f"{'='*120}")
for cfg_label, H, avg_axis, feat_label in configs:
    print(f"\n  Feature: {feat_label}")
